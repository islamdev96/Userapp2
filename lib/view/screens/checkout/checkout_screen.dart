...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Container _placeOrderButton(BuildContext context, bool _selfPickup, OrderProvider order, bool _kmWiseCharge, double _deliveryCharge) {
    return Container(
      width: 1170,
      child: CustomButton(
          ...
          if (!_selfPickup && order.addressIndex == -1) {
            ...
            if(_isCashOnDeliveryActive && Provider.of<OrderProvider>(context, listen: false).paymentMethodIndex == 0) {
              ...
              String _url = "customer_id=${Provider.of<ProfileProvider>(context, listen: false).userInfoModel!.id}"
                  "&&callback=${AppConstants.BASE_URL}${RouteHelper.orderSuccessful}&&order_amount=${widget.amount}";

              String _webUrl = "customer_id=${Provider.of<ProfileProvider>(context, listen: false).userInfoModel!.id}"
                  "&&callback=$protocol//$hostname:$port${RouteHelper.ORDER_WEB_PAYMENT}&&order_amount=${widget.amount}&&status=";
              ...
              if(ResponsiveHelper.isWeb()) {
                ...
                Navigator.pushReplacementNamed(
                  context,
                  RouteHelper.getPaymentRoute(page: 'checkout',  selectAddress: selectedUrl, placeOrderBody: _placeOrderBody),
                );
              }
            }
          }
